...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        if (epoch + 1) % display_step == 0:
            ...
            weight_3 = round(sess.run(W_3), 5)
            bias = round(sess.run(b), 5)
            loss_list.append(round(c, 5))
            # weight_list.append(weight_1)
            bias_list.append(bias)
            print("Epoch : ", "%04d" % (epoch + 1), "loss = ", "{:.9f}".format(c),
                  "W_1 = ", weight_1, "W_2 = ", weight_2, "W_3 = ", weight_3, " b = ", bias)

    print("Optimization Finished!")
    w_1, w_2, w_3, b_ = sess.run([W_1, W_2, W_3, b])
    training_loss = sess.run(loss, feed_dict={X: train_X, Y: train_Y})
    print("Training loss = ", training_loss, "W_1 = ", w_1, "W_2 = ", w_2, "W_3 = ", w_3, "b = ", b_,
          "\n")
    ...
    plt.plot(train_X, w_3 * numpy.power(train_X, 3) + w_2 * numpy.power(train_X, 2) + w_1 * train_X + b_,
             label="Fitted line")
