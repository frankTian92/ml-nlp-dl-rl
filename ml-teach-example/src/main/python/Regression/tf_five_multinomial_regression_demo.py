...
W_4 = tf.Variable(rng.randn(), name="weight_4")
W_5 = tf.Variable(rng.randn(), name="weight_4")
b = tf.Variable(rng.randn(), name="bias")

# 构造线性模型——增加x*x项
pred = tf.add(tf.multiply(X, W_1), b)
pred = tf.add(tf.multiply(tf.square(X), W_2), pred)
pred = tf.add(tf.multiply(tf.pow(X, 3), W_3), pred)
pred = tf.add(tf.multiply(tf.pow(X, 4), W_4), pred)
pred = tf.add(tf.multiply(tf.pow(X, 5), W_5), pred)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        if (epoch + 1) % display_step == 0:
            ...
            weight_1 = round(sess.run(W_1), 7)
            weight_2 = round(sess.run(W_2), 7)
            weight_3 = round(sess.run(W_3), 7)
            weight_4 = round(sess.run(W_4), 7)
            weight_5 = round(sess.run(W_5), 7)
            bias = round(sess.run(b), 7)
            loss_list.append(round(c, 7))
            # weight_list.append(weight_1)
            bias_list.append(bias)
            print("Epoch : ", "%04d" % (epoch + 1), "loss = ", "{:.9f}".format(c),
                  "W_1 = ", weight_1, "W_2 = ", weight_2, "W_3 = ", weight_3, "W_4 = ", weight_4,"W_5 = ", weight_5, " b = ", bias)

    print("Optimization Finished!")
    w_1, w_2, w_3, w_4, w_5, b_ = sess.run([W_1, W_2, W_3, W_4, W_5, b])
    training_loss = sess.run(loss, feed_dict={X: train_X, Y: train_Y})
    print("Training loss = ", training_loss, "W_1 = ", w_1, "W_2 = ", w_2, "W_3 = ", w_3, "W_4 = ", w_4,"W_5 = ", w_5, "b = ", b_,
          "\n")
    ...
    plt.plot(train_X, w_5 * numpy.power(train_X, 5) + w_4 * numpy.power(train_X, 4) + w_3 * numpy.power(train_X, 3)
             + w_2 * numpy.power(train_X,  2) + w_1 * train_X + b_,
             label="Fitted line")
