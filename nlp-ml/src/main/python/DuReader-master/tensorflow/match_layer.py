...
class MatchLSTMLayer(object):
    ...
    def __init__(self, hidden_size, W_con1, b_con1):
        self.hidden_size = hidden_size
        self.W_con1 = W_con1
        self.b_con1 = b_con1
...
class AttentionFlowMatchLayer(object):
    ...
    def match(self, passage_encodes, question_encodes, p, q, p_length, q_length, sep_q_encodes_1):
        """
        Match the passage_encodes with question_encodes using Attention Flow Match algorithm
        """
        with tf.variable_scope('bidaf'):
            p_mask = tf.sequence_mask(p_length, tf.shape(p)[1], dtype=tf.float32, name='passage_mask')
            q_mask = tf.sequence_mask(q_length, tf.shape(q)[1], dtype=tf.float32, name='question_mask')
            q_mask_1 = tf.sequence_mask(q_length, tf.shape(q)[1], dtype=tf.float32, name='question_mask_1')

            sim_matrix_2 = tf.matmul(passage_encodes, question_encodes, transpose_b=True)
            sim_mask_2 = tf.matmul(tf.expand_dims(p_mask, -1), tf.expand_dims(q_mask, -1), transpose_b=True)
            sim_matrix_2 -= (1 - sim_mask_2) * 1e30
            ...
            p_mask_2 = tf.expand_dims(p_mask, -1)
            context2question_attn_2 *= p_mask_2
            question2context_attn_2 *= p_mask_2

            sim_matrix_1 = tf.matmul(passage_encodes, sep_q_encodes_1, transpose_b=True)
            sim_mask_1 = tf.matmul(tf.expand_dims(p_mask, -1), tf.expand_dims(q_mask_1, -1), transpose_b=True)
            sim_matrix_1 -= (1 - sim_mask_1) * 1e30
            ...
            question2context_attn_1 = tf.tile(tf.matmul(b_1, passage_encodes),
                                              [1, tf.shape(passage_encodes)[1], 1])
            p_mask_1 = tf.expand_dims(p_mask, -1)
            context2question_attn_1 *= p_mask_1
            question2context_attn_1 *= p_mask_1

            concat_outputs = tf.concat([passage_encodes,
                                        context2question_attn_1,
