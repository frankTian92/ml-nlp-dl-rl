...
# ——————————————————定义神经网络变量——————————————————
def lstm(batch,scope_name):  # 参数：输入网络批次数目
    w_in = weights['in']
    b_in = biases['in']
            ...
            while (end < len(train_x)):
                ...
                if step % 10 == 0:
                    ...
                    print("保存模型：", saver.save(sess,
                                              'D:\\myProject\\ml-nlp-dl-rl\\nlp-ml\\src\\main\\resources\\stock_predict\\model_1\\stock.model'))
                step += 1
...
def prediction():
    ...
    # tf.Graph()
    pred, _ = lstm(1,"predict")  # 预测时只输入[1,time_step,input_size]的测试数据
    saver = tf.train.Saver(tf.global_variables())
    with tf.Session() as sess:
        # 参数恢复
