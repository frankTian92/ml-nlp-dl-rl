...
//            }
            ...
            for (int i = 0; i < list.size() - 1; i++) {
                ...
                for (int j =i+1;j<list.size();j++){
                    ...
                    if (dis_decay<1.0){
                        dis_decay = dis_decay/5.0;
                    }
                    int after = list.get(j);
                    trans_p_Nd.put(before, after, trans_p_Nd.getInt(before, after) +1.0+ dis_decay);
        ...
        trans_p_arr = Transforms.sigmoid(trans_p_arr);
//        ReadWriteTxtUtil.writeTxtArr1("E:\\工作\\九州通\\九州通数据分析\\第一站点概率.txt",start_p_Nd,dictSize);
    ...
    public int[] computer(List<Integer> ops_seq){
        ...
        return viterbi.computer(ops,trans_p_arr).getSeqRank();
    }
